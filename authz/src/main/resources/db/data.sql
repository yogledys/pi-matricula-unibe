...
CREATE OR REPLACE VIEW auth.v_authorities_users as 
SELECT u.name as user, r.name as role, a.name as authority
FROM auth.users u
JOIN auth.users_roles us on (u.user_id = us.user_id)
JOIN auth.roles r on (us.role_id = r.role_id)
JOIN auth.roles_authorities ra on (r.role_id = ra.role_id)
